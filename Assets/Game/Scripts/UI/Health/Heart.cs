...
{
    [SerializeField] private float _lerpDuratoin;
    ...
    public void ToFill()
    {
        StartCoroutine(Filling(0, 1, _lerpDuratoin, Fill));
    }
    ...
    private IEnumerator Filling(float startValue, float endValue, float duratoin, UnityAction<float> lerpingEnd)
    {
        ...
        while (elapsed < duratoin)
        {
            nextValue = Mathf.Lerp(startValue, endValue, elapsed / duratoin);
